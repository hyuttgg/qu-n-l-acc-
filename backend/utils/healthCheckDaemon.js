...
function startHealthCheckDaemon() {
  ...
  // Self Keep-Alive Ping (prevents Render Free tier from sleeping every 15 mins)
  const https = require('https');
  const keepAliveUrl = (process.env.BACKEND_PUBLIC_URL || 'https://quan-ly-acc-viet-nam.onrender.com').trim().replace(/\/+$/, '');
  setInterval(() => {
