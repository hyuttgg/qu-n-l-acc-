 ...
 * Includes $O(1)$ fast memoization cache to prevent repeated regex & string allocations on each render tick
 */
...
export const resolveItemImage = (category: string, name: string): string => {
  ...
  const finalUrl = folder ? `/ảnh/${folder}/${normalizedName}.webp` : '';
  imageResolutionCache.set(cacheKey, finalUrl);
  return finalUrl;
};
