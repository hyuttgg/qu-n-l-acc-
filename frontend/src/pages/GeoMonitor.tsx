...
export const GeoMonitor: React.FC = () => {
  ...
  // 2. Setup Shared Socket.IO listeners (No duplicate socket connection)
  useEffect(() => {
    if (!socket) return;
    ...
    const handleUserOnline = (onlineUser: MapUser) => {
      setUsers((prev) => {
    ...
    const handleUserOffline = (offlineData: { id: string; username: string }) => {
      setUsers((prev) =>
    ...
    const handleUserMove = (updatedUser: MapUser) => {
      setUsers((prev) => {
    ...
    socket.on('user_move', handleUserMove);

    return () => {
      clearInterval(pingInterval);
      socket.off('user_online', handleUserOnline);
      socket.off('user_offline', handleUserOffline);
      socket.off('user_move', handleUserMove);
    };
  }, [socket]);
